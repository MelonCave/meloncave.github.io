...
A new public repository was created on GitHub at [https://github.com/AncientGuy/PKM](https://github.com/AncientGuy/PKM). This repository will house the entire PKM system, including Markdown notes, automation scripts, configuration files, and the mdBook source. Initializing the repository with a README.md file, [a .gitignore file](https://github.com/groda/the_ultimate_gitignore_guide) (configured for Python, Node.js, and Rust build artifacts), and a clear directory structure (/notes, /scripts, /book\_src) is the first task.  

We will use [mdBook, a command line tool that creates books using notes written in Markdown](https://rust-lang.github.io/mdBook/) starting out, with an eye toward becoming proficient enough in understanding Rust language as well as  [the Rust language dev community](https://github.com/rust-lang/rust). At a minimum, this means proficiency in rustup and tools that rustup manages such as the rustc Rust compiler, cargo Rust's build tool/package manager, rust-std Rust standard library, rust-docs local copies of the Rust documentation, rustfmt for automatically formatting Rust code according to community standards, rust-analyzer language server code completion and refactoring for IDEs and editors, and especially the [Clippy code analyser/linter detecting mistakes](https://doc.rust-lang.org/clippy/) and the [mdBook dev community and mdBook ***issues***](https://github.com/rust-lang/mdBook/issues).

The point of choosing mdBook, as an open source foundation of our PKM, is that we want to eventually develop enough proficiency in mdBook and that Rust, that we can seriously [entertain thoughts of of contributing](https://github.com/rust-lang/mdBook/blob/master/CONTRIBUTING.md) ... but *implicit in the task of setting up this repository is about* ***the BEGINNING*** *of a sorting, sifting, prioritizing methodology for topics that we care about that will characterize the intelligence gathering side of this PKM, ie the immediate REASON for this work is not about Rust or PKE, but rather about leveling up the PKM game is improving the structure of personal intelligence gathering*.  
