# Domain-Specific Algorithms and Foundation Models
...
Frankly, this level of knowledge engineering is still mostly about [meta-theoretic algorithmic vaporware](https://g.co/gemini/share/e8d368e19d4e) but we are start to theorize about how we might build something like a [portfolio of 100 distinct, domain-specific algorithm categories for the purposes of triggering algorithmic thinking and guide foundation model development](https://docs.google.com/document/d/12dyVz_ERIbdq96ldO6YBMg_l6LVtpOJLvRqoyZeKjAo/edit?usp=sharing). Scientific exploration is largely a chicken-and-egg sort of cyclic process in which the foundation models spur us to think about new algorithmic approaches and new algorithmic approaches spur us to think about how we might sense phenomena, gather data and wrangle that data into useful models. *So in addition to discussing algroithmic vaporware, we also describe the motivation for our [curated portfolio of 100 Domain-Specific Foundational Model Concepts](#table-2-curated-portfolio-of-100-foundational-model-concepts) that people might see emerging.*

### Key Points
- Research suggests that while scaling large language models (LLMs) has driven impressive gains in capabilities like pattern recognition and text generation, it faces inherent limitations such as diminishing returns on performance, escalating computational costs, and persistent issues like hallucinations and poor generalization.
- Leading experts, including Yann LeCun and Gary Marcus, argue that scaling alone cannot achieve advanced AI goals like robust reasoning or artificial general intelligence (AGI), as LLMs lack fundamental abilities such as true understanding of the physical world or abstract planning.
- Evidence leans toward the need for innovative algorithms, such as hybrid neurosymbolic approaches or world-modeling architectures, to overcome these barriers, though debates continue on the exact path forward.
- The topic remains controversial, with some stakeholders emphasizing practical utility from scaling while others highlight sustainability and reliability concerns, underscoring a balanced view that acknowledges both progress and gaps.

### Limitations of Scaling LLMs
Scaling LLMs involves increasing model size, data volume, and compute, but experts highlight why this approach plateaus. For instance, as models grow, they require exponentially more resources, yet improvements in areas like reasoning or reliability slow down. Issues like data scarcity—high-quality training data is finite and often proprietary—exacerbate this, leading to biases or "model collapse" where LLMs trained on AI-generated content degrade in quality. Additionally, environmental costs, including massive energy consumption, make unchecked scaling unsustainable. These factors suggest that while scaling has boosted fluency in language tasks, it doesn't address core deficiencies in handling real-world complexity.

### Expert Perspectives on Alternative Paths
Prominent figures advocate shifting focus to new algorithms. Yann LeCun, Meta's Chief AI Scientist, emphasizes that LLMs operate via statistical patterns rather than genuine comprehension, failing to model high-dimensional realities like physics or causality. He proposes architectures like Joint Embedding Predictive Architecture (JEPA) to build hierarchical world models through sensorimotor learning. Gary Marcus critiques pure scaling for not resolving hallucinations or reasoning failures, pushing for neurosymbolic hybrids that combine neural networks with symbolic logic for better reliability. Other voices, such as those in recent papers, echo this by calling for multi-modal integrations or formal verification tools to enhance trustworthiness.

### Implications for AI's Future
The consensus among critics is that the "larger future" of AI lies in algorithmic innovation rather than endless scaling. This could involve developing systems with persistent memory, better generalization, and ethical safeguards. While scaling has practical applications today, like in coding or content generation, transitioning to diverse approaches may democratize AI development and reduce reliance on massive compute infrastructures.

---

### Exploring the Boundaries of Large Language Models: Why Scaling Alone Falls Short and the Case for Algorithmic Innovation

Large language models (LLMs) have revolutionized artificial intelligence, powering applications from chatbots to automated writing assistants. However, a growing chorus of leading experts contends that simply scaling these models—by increasing parameters, training data, and computational power—is insufficient for achieving transformative advancements, such as artificial general intelligence (AGI) or reliable real-world reasoning. This perspective, rooted in empirical observations and theoretical critiques, posits that the future of AI hinges on developing novel algorithms rather than perpetuating the scaling paradigm. In this comprehensive survey, we delve into the thinking of key experts, examine the limitations of scaling, review supporting evidence from recent research, and outline proposed alternatives. Drawing from interviews, papers, and public discussions, we aim to provide a balanced, evidence-based overview that highlights both the achievements of LLMs and the imperative for innovation.

#### The Foundations of the Scaling Hypothesis and Its Appeal
The scaling hypothesis, popularized in the wake of models like GPT-3, suggests that increasing model size and training data predictably enhances performance across tasks. This idea stems from empirical "scaling laws," which demonstrate that loss (a measure of model error) decreases smoothly with more compute. Proponents argue that this path has yielded "emergent" abilities, where larger models unexpectedly excel in complex tasks like code generation or summarization. For instance, scaling has enabled LLMs to handle vast datasets, leading to fluent outputs that mimic human-like language.

Yet, as models have grown—from billions to trillions of parameters—diminishing returns have become evident. Performance gains slow, while costs soar exponentially in terms of energy, infrastructure, and data acquisition. Experts now question whether scaling can bridge gaps in fundamental capabilities, such as true reasoning or understanding causality, without architectural overhauls.

#### Key Limitations of Scaling LLMs
Scaling has undeniable benefits, but it amplifies inherent flaws rather than resolving them. Below, we outline major limitations, supported by expert analyses and research findings.

| Limitation | Description | Examples/Evidence | Why Scaling Doesn't Help |
|------------|-------------|-------------------|--------------------------|
| **Diminishing Returns and Plateaus** | As models scale, performance improvements taper off, especially in reasoning or novel tasks. | Recent models like GPT-4 show slower gains compared to earlier jumps; papers note "scaling walls" where progress halts despite more compute. | Additional parameters reinforce patterns in training data but don't foster new cognitive structures like abstraction or planning. |
| **Data Scarcity and Quality Issues** | High-quality, diverse data is limited; over-reliance on internet-scraped text leads to biases and "model collapse." | Training on AI-generated content degrades outputs; proprietary data in fields like chip design restricts EDA applications. | Scaling exacerbates this by demanding ever-more data, but finite human-generated content can't keep pace without synthetic augmentation risks. |
| **Hallucinations and Unreliability** | LLMs generate plausible but false information due to probabilistic nature. | Common in factual queries; even advanced models fail on tasks like Tower of Hanoi beyond training distribution. | Scaling improves fluency but not truthfulness, as it relies on statistical sampling rather than verifiable logic. |
| **Lack of True Reasoning and World Models** | Models excel at pattern matching but struggle with causal, temporal, or physical understanding. | Failures in puzzles requiring multi-step planning; no internal representation of 3D physics or object permanence. | Larger models still operate on fixed-token prediction, inefficient for System 2 reasoning (deliberative thinking). |
| **Resource and Sustainability Challenges** | Massive compute demands are environmentally and economically unsustainable. | Training costs billions; energy use rivals small countries. | Scaling intensifies these without proportional benefits, ignoring efficiency gains from alternative designs. |
| **Bias, Toxicity, and Ethical Risks** | Inherited from training data, leading to discriminatory outputs or jailbreak vulnerabilities. | Amplifies societal biases; safeguards often bypassed. | More data can entrench biases; scaling doesn't inherently add ethical reasoning mechanisms. |
| **Catastrophic Forgetting and Generalization** | Fine-tuning erases prior knowledge; poor transfer to out-of-distribution tasks. | Sequential training degrades old skills; fails on evolutionary or acquisition aspects of human cognition. | Scaling aids average performance but not robust generalization beyond trained scenarios. |

These limitations underscore that scaling reinforces existing paradigms without addressing structural deficits, such as the absence of persistent memory or multi-modal integration.

#### Expert Thinking: Voices Advocating for Algorithmic Shifts
Leading figures in AI have articulated why scaling falls short and what alternatives might entail. Their views, often shared in interviews, papers, and social media, reflect a consensus on the need for innovation.

| Expert | Key Arguments Against Scaling | Proposed Alternatives | Supporting Evidence/Quotes |
|--------|-------------------------------|-------------------------|----------------------------|
| **Yann LeCun (Meta Chief AI Scientist)** | LLMs lack System 2 reasoning, world models, and high-dimensional representation; scaling can't simulate physical realities or enable true prediction. | Develop JEPA for hierarchical abstractions via sensorimotor learning; focus on open-source models for diversity. | "LLMs are incapable of inventing new things—they regurgitate statistics of text." Predicts LLMs obsolete in five years. |
| **Gary Marcus (AI Researcher, Author)** | Pure scaling hits walls in reliability and reasoning; LLMs can't generalize or avoid hallucinations without hybrids. | Neurosymbolic AI combining neural and symbolic methods; innate constraints for better world models. | "Scaling is over, the bubble may be deflating, LLMs still can't reason." Notes even scaling proponents like Rich Sutton are shifting. |
| **Rich Sutton (Turing Award Winner)** | Initially advocated scaling per "Bitter Lesson," but now recognizes limits of pure prediction. | Reinforcement learning with world models; move beyond LLMs. | Podcast discussions highlight agreement on needing more than scaling; shift from overstating scaling's role. |
| **Fei-Fei Li (AI Pioneer)** | Language is insufficient; nature lacks "words in the sky," requiring 3D physical understanding. | Multi-modal models grounded in physics and sensory data. | "There's no language out there in nature... There is a 3D world that follows laws of physics." |
| **Melanie Mitchell (Santa Fe Institute)** | No strong evidence for emergent abstract world models in LLMs; scaling doesn't yield robust intelligence. | Explore limited domain models (e.g., Othello) but prioritize broader innovations. | "The claims of emergent abstract world models in LLMs are not yet supported by strong evidence." |

These experts emphasize that LLMs' token-based prediction is akin to rote memorization, inefficient for planning or innovation. Marcus, for example, has consistently predicted scaling's failure [to scale infinitely] since 2022, validated by recent model plateaus.

#### Evidence from Recent Research
Empirical studies reinforce these views. Apple's "Illusion of Thinking" paper demonstrates LLMs' failure on reasoning benchmarks like Tower of Hanoi, where even scaled "reasoning models" (e.g., o3) collapse beyond simple cases, showing chain-of-thought as ineffective for true logic. An arXiv survey on AI in chip design highlights scaling's challenges in multi-modal data, advocating hybrid LLM-LCM systems with formal verification. Another primer details hallucinations, biases, and forgetting, noting that optimal compute budgets favor balanced data-model scaling over sheer size.

Public discourse on platforms like X (formerly Twitter) echoes this, with Marcus noting the field's awakening to scaling's limits and calling for reflection on overhyping.

#### Pathways Forward: Toward New Algorithms
The path ahead involves algorithmic diversity. Proposals include:
- **Neurosymbolic Hybrids**: Integrate symbolic rules for explainability and reliability.
- **World Models and Multi-Modality**: Build systems like JEPA that learn from video or sensory data for physical understanding.
- **Hybrid Verification**: Combine LLMs with formal tools for safety-critical tasks.
- **Efficient Architectures**: Explore Liquid Foundation Models or specialized hardware to reduce scaling dependency.

Challenges remain, including data privacy and evaluation metrics, but experts like LeCun advocate open-sourcing to accelerate progress. While scaling has utility in narrow applications, the broader AI future demands innovation to achieve trustworthy, general-purpose systems.

This survey illustrates that while scaling has propelled AI forward, its limitations necessitate a pivot to algorithmic creativity, as articulated by leading thinkers.

### Key Citations
- [AI 'Godfather' Yann LeCun: LLMs Are Nearing the End, but Better AI Is Coming](https://www.newsweek.com/nw-ai/ai-impact-interview-yann-lecun-llm-limitations-analysis-2054255)
- [Game over for pure LLMs. Even Turing Award Winner Rich Sutton has gotten off the bus](https://garymarcus.substack.com/p/game-over-for-pure-llms-even-turing)
- [A knockout blow for LLMs?](https://garymarcus.substack.com/p/a-knockout-blow-for-llms)
- [Revolution or Hype? Seeking the Limits of Large Models in AI Hardware Design](https://arxiv.org/pdf/2509.04905.pdf)
- [A Primer on Large Language Models and their Limitations](https://arxiv.org/pdf/2412.04503.pdf)
- [Scaling Laws for LLMs: From GPT-3 to o3](https://cameronrwolfe.substack.com/p/llm-scaling-laws)
- [What does the future of AI look like if we hit the LLM scaling wall?](https://medium.com/data-science-collective/what-does-the-future-of-ai-look-like-if-we-hit-the-llm-scaling-wall-6323b8f72e79)
- [The end of AI scaling may not be nigh: Here's what's next](https://venturebeat.com/ai/the-end-of-ai-scaling-may-not-be-nigh-heres-whats-next)
- [Scaling is over, the bubble may be deflating, LLMs still can't reason](https://garymarcus.substack.com/p/scaling-is-over-the-bubble-may-be)
- [Yann LeCun on the limitations of LLMs and the future of AI](https://www.linkedin.com/posts/robrogowski_why-cant-ai-make-its-own-discoveries-activity-7370499127991832576-vd06)

The prevailing narrative of artificial intelligence ***has been*** [NOTICE THE TENSE OF THE VERB] dominated by foundation models that emulate human cognition, primarily through language, eg LLMs, and image generation, eg gaussian splatting. While transformative, especially when applied in a multi-modal sense, this represents only the first wave of a far larger and more profound technological revolution which is about entirely different problem-solving algorithms, from vastly different bodies of knowledge, applied to new areas of knowledge and entirely new ways of sensing [and making sense of] new phenomena that extend beyond human scale. In other words, human thinking has been dominated by human scale sensory experience in the same ways that building construction methods and architectures were once limited by what was possible with human physical energy even when that energy involved coordination of large and scaled populations human laborers -- until humans became smarter about engineering, architecture and things like leverage, there were significant limits to what kinds of structures were even possible. Similarly, human knowledge engineering must transcend past human experience as human more deeply explore new realms that have been beyond direct hands-on human experience. 

Large language models (LLMs) have substantially advanced machine learning research, including natural language
processing, computer vision, data mining, etc., yet they still exhibit critical limitations in explainability, reliability, adaptability, and
extensibility. In this paper, we overview a promising learning paradigm, i.e., Modular Machine Learning (MML), as an essential approach
toward new-generation LLMs capable of addressing these issues. We begin by systematically and comprehensively surveying the
existing literature on modular machine learning, with a particular focus on modular data representation and modular models. Then, we
propose a unified MML framework for LLMs, which decomposes the complex structure of LLMs into three interdependent components:
modular representation, modular model, and modular reasoning. Specifically, the MML paradigm discussed in this article is able to: i)
clarify the internal working mechanism of LLMs through the disentanglement of semantic components; ii) allow for flexible and
task-adaptive model design; iii) enable an interpretable and logic-driven decision-making process. We further elaborate a feasible
implementation of MML-based LLMs via leveraging advanced techniques such as disentangled representation learning, neural
architecture search and neuro-symbolic learning. Last but not least, we critically identify the remaining key challenges, such as the
integration of continuous neural and discrete symbolic processes, joint optimization, and computational scalability, present promising
future research directions that deserve further exploration. Ultimately, we believe the integration of the MML with LLMs has the potential
to bridge the gap between statistical (deep) learning and formal (logical) reasoning, thereby paving the way for robust, adaptable, and
trustworthy AI systems across a wide range of real-world applications.

This report posits that the next, and arguably more significant, frontier for foundation models lies in their application to fundamental scientific discovery. The emergence of these Scientific Foundation Models (SciFMs) is the catalyst that fully realizes the "Fourth Paradigm" of science—a new era where data-intensive, AI-driven exploration becomes a primary mode of discovery, standing alongside the traditional pillars of theory, experimentation, and simulation.
