...
One example of 2nd mover advantage is using the [USPTO's PatentsView bulk downloads](https://patentsview.org/what-is-patentsview) to explore publicly accessible patent research data sets with detailed documentation ... PatentsView database tables can be bulk downloaded as individual files in a tab- delimited format for programmers and researchers who prefer to work with the data in their native environments -- but the point is that this data can be / has been / is being used by problem-solving research and development teams in industry to rapidly overcome the tendency to solve problems too slowly -- the most expeditious way is to fully utilize 2nd mover advantage to use inventions that have already worked elsewhere, probably are even off-patent by now, and eliminate problems immediately ... rather than trying to learn, starting from scratch.
