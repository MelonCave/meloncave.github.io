...
Ai2 has formalized this philosophy under its "MORE THAN Open" initiative, ie because ***somebody*** has hijacked the word *open* for their AI brand. The core premise is that open weights are insufficient for science. To treat AI as a scientific instrument, researchers must be able to inspect the "calibration" of the instrument—the data it was trained on. If an astronomer uses a telescope, they must know the properties of the lens; if a biologist uses an AI agent, they must know the composition of its training corpus.
